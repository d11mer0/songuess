      ...
      <nav className={styles.navbar}>
        <div className={styles.navTop}>
          <div className={styles.navLeft}>
            <Link to="/" className={styles.logo}>MusicApp</Link>
          ...
          <div className={styles.navRight}>
            {isAuthenticated ? (
              <div className={styles.userMenu} onClick={() => setIsDropdownOpen(!isDropdownOpen)}>
                {user?.avatar && (
                  <img src={user.avatar} alt="Avatar" className={styles.avatar} referrerPolicy="no-referrer"/>
                )}
                <span className={styles.login}>{user?.login}</span>
                <span className={`${styles.arrow} ${isDropdownOpen ? styles.rotate : ""}`}>&#9662;</span>

                {isDropdownOpen && (
                  <div className={styles.dropdownMenu}>
                    <Link to="/user/me" className={styles.dropdownItem}>Edit Profile</Link>
