  ...
  return (
    <div className={styles.searchContainer}>
      <input
        ...
        placeholder="Пошук..."
        onBlur={() => {
          timeoutRef.current = window.setTimeout(() => setIsDropdownOpen(false), 300);
        }}
      ...
      {isDropdownOpen && options.length > 0 && (
        <ul className={styles.dropdownList}>
          {options.map((item) => (
            ...
            >
              <span className={styles.mainText}>{String(item[optionLabel])}</span>
              {getSubtext && <span className={styles.subText}> – {getSubtext(item)}</span>}
            </li>
          ))}
        </ul>
      )}
