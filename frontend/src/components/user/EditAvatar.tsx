...
const EditAvatar: React.FC<EditAvatarProps> = ({ user, show, onClose }) => {
  ...
  const [updateAvatar, { error, isLoading, reset }] = useUpdateAvatarMutation();
  ...
  if (isLoading) {
    return (
      <CustomModal isOpen={show} title="Edit Avatar" onClose={handleClose}>
        <Loader />
      </CustomModal>
    );
  }
  ...
  return (
    <CustomModal isOpen={show} onClose={handleClose} title="Edit Avatar">
      ...
      />
      <DragAndDropZone onFileDrop={handleAvatarChange} setErrorMessage={setErrorMessage} />
      {(error as any)?.data?.message || errorMessage ? (
        <AuthFormError error={(error as any)?.data?.message || errorMessage} />
      ) : null}
      <div className={styles.uploadButtonContainer}>
        <Button variant="primary" onClick={handleUpdate} disabled={isLoading} width={"40%"}>
