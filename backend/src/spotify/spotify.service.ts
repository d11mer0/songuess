...
export class SpotifyService {
  ...
  constructor(private readonly httpService: HttpService) {
    ...
    if (!this.clientId || !this.clientSecret) {
      throw new InternalServerErrorException('Spotify credentials are missing in environment variables.');
    }
  }
  ...
  async getAccessToken(): Promise<string> {
    ...
    try {
      ...
      if (!this.accessToken) {
        throw new BadRequestException('Failed to retrieve Spotify access token.');
      }

      return this.accessToken;
    } catch (error) {
      throw new BadRequestException('Error fetching Spotify access token.');
    }
  }
