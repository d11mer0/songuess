...
export class AuthService {
  ...
  async login({ login, password }: LoginDto) {
    ...
    if(!user.password) {
      throw new BadRequestException('This email verified via Google verification. \n Use "forgot password" link to create password');
    }

    if (!user.isVerified) {
      throw new ForbiddenException('Email is not verified. Please check your email.');
    }

    await this.hashService.comparePasswords(password, user.password);
    return this.tokenService.generateTokens(user.id, user.login, user.email);
  }
  ...
  async sendTokenEmail(email: string, type: 'EMAIL_VERIFICATION' | 'PASSWORD_RESET') {
    const user = await this.prisma.user.findUnique({ where: { email } });

    if (!user || (type === 'EMAIL_VERIFICATION' && user.isVerified)) {
      throw new NotFoundException('User with this email does not exist or is already verified');
    }
    ...
    await this.prisma.token.deleteMany({ where: { userId: user.id, type } });

    await this.prisma.token.create({
      data: { userId: user.id, token, type, expiresAt: new Date(Date.now() + ms(type === 'EMAIL_VERIFICATION' ? '1d' : '1h')) },
    });
    ...
    return { message: `${type === 'EMAIL_VERIFICATION' ? 'Verification' : 'Password reset'} email sent` };
  }
  ...
  async googleAuth(idToken: string) {
    ...
    const user = await this.userService.findOrCreateGoogleUser(payload as { email: string; name: string; sub: string; picture: string });
